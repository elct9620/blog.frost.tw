...
基本上，我們會把測試放到專案裡面大多是為了要避免一些人會不小心犯失誤。所以透過定義一些自動化的程式，來確定我們的實作跟「規格」是相同的。
...
像是在寫 Model 的時候我是會避免放太多邏輯在裡面的，除非專案真的很小才會考慮直接寫在裡面。當你的專案變複雜的時候，我們常常會要做很多步驟的處理才能完成一件事情，這其實就算是一種訊號告訴我們需要把這些東西拆到一個獨立的物件上，然後我們就可以專心用測試檢查這個處理的流程（通常我們就會叫這類物件 Service Object）
